...
export class LockPlayerTeamSettingsScene extends ActionUIScene {
	...
	constructor(
		...
		const currentLock = this.lockPlayerService.getLockSettings(
			subjectTeam.id,
		);
		...
		this.addButton(
			...
			"textures/edu_tools/ui/lock_player/settings",
		);
		if (!currentLock?.playerBound) {
			this.addButton(
				...
				"textures/edu_tools/ui/lock_player/update_center",
			);
		}
		this.addButton(
			...
			"textures/edu_tools/ui/lock_player/teleport_to_center",
		);
		this.addButton(
			...
			() => {
				...
				this.player.sendMessage({
					translate:
						"edu_tools.message.lock_player_team_settings.delete_lock",
				});
