...
import { ActionUIScene, ModalUIScene } from "../scene_manager/ui-scene.ts";
...
export class LockPlayerScene extends ActionUIScene {
  ...
  constructor(
    ...
    this.addButton(
      ...
      (): void => {
        ...
        context.setData("team_filter", (team: Team): boolean => {
          for (const memberId of team.memberIds) {
            const isPlayerExempted = lockPlayerService.isPlayerExempted(
              memberId,
            );
            if (!isPlayerExempted) {
              return true;
      ...
      "textures/edu_tools/ui/lock_player/add_team_lock",
    );
    this.addButton(
      ...
      "textures/edu_tools/ui/lock_player/view_teams",
    );
