...
export class PlayerNicknameService {
  ...
  getMainButton(): ButtonConfig {
    return {
      ...
      iconPath: "textures/edu_tools/ui/main/player_nickname",
  ...
  rejectNickname(playerId: string): void {
    const nickname = this.approvalQueueStorage.get(playerId);
    if (nickname) {
  ...
  checkIfShouldOpenQueue(): boolean {
    ...
    const timeSinceLastRequest = system.currentTick -
      (this.getLastApprovalRequestTick() || 0);
  ...
  getSettings(): PlayerNicknameSettings {
    ...
    const settings = this.storage.get("settings") as
      ...
      | undefined;
    return {
      nicknamesEnabled: settings?.nicknamesEnabled ??
        defaultSettings.nicknamesEnabled,
      promptOnJoin: settings?.promptOnJoin ?? defaultSettings.promptOnJoin,
      allowCustomColors: settings?.allowCustomColors ??
        defaultSettings.allowCustomColors,
      requireApproval: settings?.requireApproval ??
        defaultSettings.requireApproval,
      customLeaveJoinMessages: settings?.customLeaveJoinMessages ??
