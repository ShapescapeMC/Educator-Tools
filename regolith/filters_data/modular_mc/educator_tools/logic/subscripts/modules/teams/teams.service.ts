...
export class TeamsService implements Module {
	...
	getMainButton(): ButtonConfig {
		return {
			...
			iconPath: "textures/edu_tools/ui/main/teams_management",
	...
	addPlayerToTeam(teamId: string, playerId: string): boolean {
		...
		if (
			team.maximumMembers && team.memberIds.length >= team.maximumMembers
		) {
			throw new Error(
	...
	removePlayerFromTeam(teamId: string, playerId: string): boolean {
		...
		if (
			team.minimumMembers && team.memberIds.length <= team.minimumMembers
		) {
			throw new Error(
	...
	getTeam(teamId: string): Team | undefined {
		...
		if (teamId === TeamsService.ALL_PLAYERS_TEAM_ID) {
			...
			const playerId = teamId.replace(
				TeamsService.PLAYER_TEAM_PREFIX,
				"",
			);
